...
app.post("/user_input", (req, res) => {
  ...
  let ip = req.clientIp;
  let node1 = req.body.nodeA;
  let node2 = req.body.nodeC;
...
app.post("/up", (req, res, next) => {
  let page = req.body.page;
  let nodeA = res.body.nodeA;
  let nodeB = res.body.nodeB;
  let nodeC = res.body.nodeC;
  let relation1 = res.body.relation1;
  let relation2 = res.body.relation2;
  let relation3 = res.body.relation3;
  let weight = 0;

  //insert results to table transition --> do in /transition
  let query1 = "insert into transition (term_1, relation1, term_2, relation2, term_3, relation3, weight) " +
  "values ("+ nodeA + ", " + relation1 + ", " + nodeB + ", " + relation2 + ", " + nodeC + ", " + relation3 + ", " + weight + ");";

  //update column weight of table transition to 1
  let query = "update transition set weight = " + weight++ + " where ;";
